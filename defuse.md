Let's focus on the situations in which "AI-enabled dangerous activities" or preparations for them are pursued by an agent, recalling [[README]] (also [[information#more on acausality and non-causality]]). Essentially we want to identify and defuse such situations, although the nature of the danger can be unprecedented, moreover, never before imagined, and the extent of the risk can be such that any positive experiment (in that its outcome is the undesirable incident happening) would likely cause mass extinction.[^1] The less fundamental problem with such risk is that reasoning about it probabilistically is impossible and self-contradictory (viz. Pascal's wager, but extended beyond a frequentist interpretation). More importantly: we are making the following requirements
1. there is a class of incidents which are certified to not occur (at least not directly caused by AI) at any point in the future
2. we can reject the certification to be directly caused by "no such incident occurred" being true at every point in the future
3. there is a set of preventive actions that directly cause the validity of the certification
4. we reject the preventive power of our actions to be evidence-based, we only accept theoretical proof
5. for such proof we have to reason about the class of undesirable incidents that is not entirely defined, enumerable or even completely accessible (theoretically)
