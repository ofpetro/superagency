...
| $H$               | $q(t)=Q(t)-Q(t-1)>0$ with $Q(t)$ a finite penalty, $Q(t-1)$ the penalty received so far from this effect and action $H$ | $\bar q=Q(\infty)-Q(t-1)$ with $Q(\infty)$ a finite total penalty that this effect yields on action $H$ |
...
[^1]: By definition, future rewards. If we assume that the agent doesn't change its predictions starting with some future timestep, then this is equivalent to future value estimation for all but finitely many timesteps.
